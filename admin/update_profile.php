...
if (isset($_POST['submit'])) {
   ...
   if (!empty($name)) {
      ...
      $select_name->bindParam(1, $name, PDO::PARAM_STR); // Bind the name parameter as string
      $select_name->execute();
      if ($select_name->rowCount() > 0) {
         $message[] = 'Username already taken!';
      } else {
         $update_name = $conn->prepare("UPDATE `admin` SET name = ? WHERE id = ?");
         $update_name->bindParam(1, $name, PDO::PARAM_STR); // Bind the name parameter as string
         $update_name->bindParam(2, $admin_id, PDO::PARAM_INT); // Bind the admin_id parameter as integer
         $update_name->execute();
      }
   }
   ...
   $select_old_pass->bindParam(1, $admin_id, PDO::PARAM_INT); // Bind the admin_id parameter as integer
   ...
   if ($old_pass != $empty_pass) {
      if ($old_pass != $prev_pass) {
         ...
         if ($new_pass != $empty_pass) {
            ...
            $update_pass->bindParam(1, $confirm_pass, PDO::PARAM_STR); // Bind the confirm_pass parameter as string
            $update_pass->bindParam(2, $admin_id, PDO::PARAM_INT); // Bind the admin_id parameter as integer
            $update_pass->execute();
